import React from "react";
import { NavLink, useLocation } from "react-router-dom";
import Profile from "../assets/Profile.png"; 
import menuDB from "../assets/menuDB.png"; 
import menuCL from "../assets/menuCL.png"; 
import menuTC from "../assets/menuTC.png"; 
import AdminLogout from "../components/AdminLogout.jsx";

const AdminNavbar = () => {
  const location = useLocation();


  const isTeacherSectionActive = (isActive) => {
    return isActive || location.pathname.startsWith("/admin/AdminClassRoom");
  };

  return (
    <aside
      className="
        bg-white border border-[#e9e9e9] rounded-[20px] shadow-sm
        lg:fixed lg:top-2 lg:left-1      
        md:static md:mt-6 md:ml-8          
        flex flex-col justify-between
        w-44 lg:w-56 md:w-48 h-[85vh] 
        md:h-auto lg:min-h-[85vh] lg:max-h-[85vh]
        transition-all
      "
    >
      <div className="p-4">
        {/* ส่วนโปรไฟล์ */}
        <div className="flex items-center gap-3 mb-3">
          <img
            className="w-10 h-10 rounded-full md:w-9 md:h-9"
            src={Profile}
            alt="Profile"
          />
          <div className="text-black text-base font-medium">แอดมิน</div>
        </div>

        <hr className="p-3 border-t border-[#e9e9e9]" />

        {/* เมนูด้านใน */}
        <nav className="flex flex-col items-center gap-2">
          <NavLink
            to="/admin/AdminHomePage"
            className={
              ({ isActive }) =>
                (isActive
                  ? "bg-[#38a738] text-white"
                  : "text-gray-700 hover:bg-gray-100") +
                " w-40 h-9 px-3 rounded-md flex items-center gap-2 text-sm " + 
                " md:w-48 md:h-10 md:px-4 md:gap-3 md:text-base " + 
                " lg:w-52 lg:h-10 lg:px-4 lg:gap-3" 
            }
          >
            {({ isActive }) => (
              <>
                <img
                  className="w-3 h-3 md:w-3.5 md:h-3.5 transition-all duration-200"
                  src={menuDB}
                  alt="menuDB"
                  style={{
                    filter: isActive
                      ? "brightness(0) invert(1)" // สีขาวตอน active
                      : "brightness(0) invert(0)", // สีดำตอนปกติ
                  }}
                />
                Dashboard
              </>
            )}
          </NavLink>

          <NavLink
            to="/admin/AdminTeachers"
            // ใช้ฟังก์ชันที่สร้างขึ้นมาใหม่
            className={({ isActive }) =>
              (isTeacherSectionActive(isActive) // 
                ? "bg-[#38a738] text-white"
                : "text-gray-700 hover:bg-gray-100") +
              " w-40 h-9 px-3 rounded-md flex items-center gap-2 text-sm " +
              " md:w-48 md:h-10 md:px-4 md:gap-3 md:text-base " +
              " lg:w-52 lg:h-10 lg:px-4 lg:gap-3"
            }
          >
            {({ isActive }) => (
              <>
                <img
                  className="w-5 h-5 md:w-5 md:h-5 transition-all duration-200"
                  src={menuCL}
                  alt="menuCL"
                  style={{
                    // ⬅️ 5. ใช้ฟังก์ชันที่สร้างขึ้นมาใหม่สำหรับไอคอนด้วย
                    filter: isTeacherSectionActive(isActive)
                      ? "brightness(0) invert(1)" // สีขาวตอน active
                      : "brightness(0) invert(0)", // สีดำตอนปกติ
                  }}
                />
                อาจารย์
              </>
            )}
          </NavLink>

          <NavLink
            to="/admin/AdminTeachingschedule"
            className={({ isActive }) =>
              (isActive
                ? "bg-[#38a738] text-white"
                : "text-gray-700 hover:bg-gray-100") +
              " w-40 h-9 px-3 rounded-md flex items-center gap-2 text-sm " +
              " md:w-48 md:h-10 md:px-4 md:gap-3 md:text-base " +
              " lg:w-52 lg:h-10 lg:px-4 lg:gap-3"
            }
          >
            {({ isActive }) => (
              <>
                <img
                  className="w-3.5 h-3.5 md:w-4 md:h-4 transition-all duration-200"
                  src={menuTC}
                  alt="menuTC"
                  style={{
                    filter: isActive
                      ? "brightness(0) invert(1)" // สีขาวตอน active
                      : "brightness(0) invert(0)", // สีดำตอนปกติ
                  }}
                />
                ตารางสอน
              </>
            )}
          </NavLink>

          <AdminLogout />
        </nav>
      </div>
    </aside>
  );
};

export default AdminNavbar;