...
const HomePage = () => {
  ...
  const processData = (records) => {
    ...
    records.forEach((record) => {
      ...
      const timeStr = record.created_at
        ? new Date(record.created_at).toLocaleTimeString("th-TH", {
            hour: "2-digit",
            minute: "2-digit",
          })
        : "-";
    ...
    const chartData = Object.keys(jsonTotals)
      .map((key) => ({
        name: key,
  ...
  const cardStyle =
    "bg-white rounded-[20px] shadow-sm border border-[#e9e9e9] p-6 flex flex-col items-center justify-center";
        ...
        <div className="space-y-6 max-w-screen-2xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className={cardStyle}>
              <span className="text-4xl font-bold text-[#1D971D]">
                {todayStats.avgFocused}%
              </span>
            ...
            <div className={cardStyle}>
              <span className="text-4xl font-bold text-[#FF3300]">
                {todayStats.avgNonFocused}%
              </span>
              <span className="text-gray-700 mt-2 font-medium">
                ไม่ตั้งใจเรียน
              </span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <div className="flex flex-col space-y-4">
              ...
              <div
                className={`${cardStyle} !p-0 !items-start relative overflow-hidden`}
              ...
              <div className="bg-white rounded-[20px] shadow-sm border border-[#e9e9e9] p-6 flex flex-col">
                ...
                <div className="space-y-3">
                  <div className="bg-green-50 border border-green-200 text-[#085E0E] px-4 py-3 rounded-xl flex justify-between items-center text-sm font-medium">
                    <span>
                      ช่วงเวลาที่ดีที่สุด {bestWorstTime.bestTime} (
                      {bestWorstTime.bestSubject})
                    </span>
                    <span className="font-bold">
                      {bestWorstTime.bestScore}%
                    </span>
                  </div>
                  ...
                  <div className="bg-red-50 border border-red-200 text-[#74393C] px-4 py-3 rounded-xl flex justify-between items-center text-sm font-medium">
                    <span>
                      ช่วงเวลาที่แย่ที่สุด {bestWorstTime.worstTime} (
                      {bestWorstTime.worstSubject})
                    </span>
                    <span className="font-bold">
                      {bestWorstTime.worstScore}%
                    </span>
                  </div>
                </div>
              </div>
